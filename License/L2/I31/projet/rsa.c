 ...
 * PubkeyPQ
 * --------------------
 * Premiere partie de la generation de la cle publique du protocole
 * RSA.
 *  Donne p et q, deux nombres premiers distincts de taille a, et leur produit n
 *
 *  Ne retourne rien mais stocke les resultats dans les variables globales
 *  ATTENTION avec n qui est modifie par d'autres fonctions
 */
    mpz_t max;
    mpz_init(max);
    int cmp1,cmp2,cont;
 ...
 * PubkeyED
 * --------------------
 * Deuxieme partie de la generation de la cle publique ainsi que la
 * generation de la cle privee.
 *  Donne e et d, tel que e un nombre premier avec phi(n) et inferieur a celui ci.
 *  d lui est l'inverse de e mod[phi(n)].
 *
 */
 ...
 * MD9T
 * --------------------
 * Prend un mpz_t et shift par la gauche jusqu'a ce que sa
 ...
 * expo
 * --------------------
 * Fait a**b[mod m], mais utilise des multiplications sucessives
 * afin de reduire la complexite.
 * 
 * Le resultat est stocke dans rex.
 * 
 */
 ...
 * txt2int
 * --------------------
 * Ouvre un fichier txt et concatene la valeur ascii de son
 ...
 * MD9
 * --------------------
 * Divise M en paquet de taille kp bit, et fait un xor entre ses
 * paquets
 * 
 * Pour la division en paquet la fonction calcule un masque et
 * utilise un AND pour selectionner.
 * 
 * ATTENTION la fonction doit imperativement d abord passer par MD9T
 * 
 */
